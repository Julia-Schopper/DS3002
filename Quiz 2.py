...
while True:
    try:
        ...
        # https://stackoverflow.com/questions/28325622/python-csv-writing-headers-only-once : headers
        with open('yahoofinance.csv', 'a') as csvfile:
            headers = ["Ticker", "Market Time", "Price"]
            csvwriter = csv.writer(csvfile)
            file_is_empty = os.stat('yahoofinance.csv').st_size == 0
            if file_is_empty:
